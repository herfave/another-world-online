...
function Simulation.new(userId)
    ...
    local STATE_TABLE = {}
    -- sanitize
    function STATE_TABLE:__newindex(i, v)
        if i == "position" then
            if typeof(v) == "Vector3" then
                if v.X ~= v.X
                or v.Y ~= v.Y
                or v.Z ~= v.Z then
                    warn("attempted to set position to a nan vector")
                    return
                end
            else
                warn("attempted to set position to non-vector")
                return
            end
        end
        rawset(self, i, v)
    ...
    self.state = {}
    setmetatable(self.state, STATE_TABLE)
     

    self.state.position = Vector3.new(0, 5, 0)
...
function Simulation:ProcessCommand(cmd)
    ...
    if self.debugModel ~= nil then
        self.debugModel:PivotTo(CFrame.new(self.state.position))
    end
    ...
    self.characterData:SetTargetPosition(self.state.position)
...
function Simulation:SetPosition(position : Vector3, teleport)
    local function resetPos()
        warn("OH NO IT WENT NAN")
        -- print(position)
        -- print(simulation.state.vel)
        local player = game.Players:GetPlayerByUserId(self.userId)
        if player then

            local serverBox = workspace:FindFirstChild(player.Name)
            if serverBox then
                position = serverBox.Position
            end
        end
    end
    ...
    -- check for nan
    -- verify pos has no NaN
    if position.X ~= position.X
    or position.Y ~= position.Y
    or position.Z ~= position.Z then
        resetPos()
    end

    self.state.position = position
    self.characterData:SetTargetPosition(self.state.position, teleport)
end
...
function Simulation:DoStepUp(pos, vel, deltaTime)
    ...
    if ground ~= nil then
        local result = {
            stepUp = self.state.position.y - stepUpNewPos.y,
            position = stepUpNewPos,
            vel = stepUpNewVel,
        }
        return result
    end
...
function Simulation:DoPlatformMove(lastGround, deltaTime)
    ...
    if lastGround and lastGround.hullRecord and lastGround.hullRecord.instance then
        ...
        if instance.Velocity.Magnitude > 0 then
            ...
            self.state.position += instance.Velocity * deltaTime
        end
    end
end
