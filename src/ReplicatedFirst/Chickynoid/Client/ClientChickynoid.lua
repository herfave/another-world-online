...
function ClientChickynoid.new(position: Vector3, characterMod: string)
    ...
    self.simulation.state.position = position
...
function ClientChickynoid:HandleNewPlayerState(stateDelta, stateDeltaTime, lastConfirmed, serverTime, playerStateFrame)
	...
	if (self.debug.useSkipResimulationOptimization == true) then
		...
		if (lastConfirmed ~= nil) then
			...
			if cacheRecord then
				...
				if (cacheRecord.stateRecord.state.position - stateRecord.state.position).magnitude < 0.05 and (cacheRecord.stateRecord.state.vel - stateRecord.state.vel).magnitude < 0.1 then
	                resimulate = false
	                -- print("skipped resim")
	            end
	        end
    ...
    if resimulate == true and stateRecord ~= nil then
        ...
        local oldPos = self.simulation.state.position
        ...
        self:SpawnDebugSphere(self.simulation.state.position, Color3.fromRGB(255, 170, 0))
        ...
        for _, command in remainingCommands do
            ...
            self:SpawnDebugSphere(self.simulation.state.position, Color3.fromRGB(255, 255, 0))
        ...
        local delta = oldPos - self.simulation.state.position
...
function ClientChickynoid:Heartbeat(command, serverTime: number, deltaTime: number)
    ...
    self:SpawnDebugSphere(self.simulation.state.position, Color3.fromRGB(44, 140, 39))
