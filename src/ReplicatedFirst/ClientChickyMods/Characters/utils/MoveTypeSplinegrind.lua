...
function module.AlwaysThink(simulation, cmd)
    ...
    local radPerc = math.clamp(speed / simulation.constants.maxAirSpeed, 0, 1)
    ...
    * CFrame.fromOrientation(0, simulation.state.angle, 0)
    ...
    if onGround then
        if onGround:FindFirstAncestor("RailVis") then
...
function module.ActiveThink(simulation, cmd)
    ...
    --         )
    --     elseif simulation.state.vel.Y >= -1 then
    ...
    local currentSpeed = simulation.constants.railgrindSpeed
    ...
    local v = simulation.constants.railgrindSpeed
    ...
    local t = math.clamp(currentPosition + step, 0, 1)-- add new distance based on speed

    local movingTo = KnitSplineHandle:GetSplineCFrame(splineObj.Name, t)
    if movingTo then
        ...
        simulation:SetPosition(movingTo.Position + (movingTo.UpVector * 3))
    ...
    --     simulation:SetPosition(simulation.state.position +
    ...
    if t <= 0 or t >= 1 or not movingTo then
        -- end of ride, dismount
        simulation.characterData:PlayAnimation(Enums.Anims.Fall, Enums.AnimChannel.Channel1, true, 0.2)
        simulation:SetPosition(simulation.state.position + (simulation.state.vel * cmd.deltaTime * 3 * flowDirection))
