...
function CLASS:SetCharacterAlignment(aligned)
	...
	local humanoid = (character ~= nil) and (character:WaitForChild("Humanoid", 3))

    if not humanoid then
        warn("Could not get humanoid")
        print(character)
        return
    end
...
function CLASS:Update()
	...
	if (humanoidRootPart ~= nil) then
		...
		local ignore = game:GetService("CollectionService"):GetTagged("CameraIgnore")
...
USER_INPUT_SERVICE.InputBegan:Connect(function(inputObject, gameProcessedEvent)
	if (gameProcessedEvent == false) and (singleton.IsEnabled == true) then
        ...
        if inputObject.KeyCode == Enum.KeyCode.G then
            singleton:SetShoulderDirection(singleton.ShoulderDirection * -1)
        end
