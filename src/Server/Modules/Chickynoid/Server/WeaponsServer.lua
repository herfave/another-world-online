...
function module:QueryShotgun(playerRecord, server, origins, directions, serverTime, debugText, raycastParams, range)
    ...
    for counter = 1, #origins do 
        ...
        if rayCastResult == nil then
            local record = {}
            record.position =  origin + dir * range
            record.origin = origin
            record.dir = dir
            table.insert(results, record)
        else
            local record = {}
            record.position = rayCastResult.Position
...
function module:Think(server, deltaTime)
    ...
    for serial, rocket in pairs(self.rockets) do
        ...
        local oldPos = rocket.position

        if oldPos == nil then
            oldPos = rocket.p
        end
        rocket.position = rocket.p + (rocket.v * rocket.c * timePassed)

        --Trace a line
        local params = RaycastParams.new()
        params.FilterType = Enum.RaycastFilterType.Include
        params.FilterDescendantsInstances = { game.Workspace.Terrain, server:GetCollisionRoot() }
        local results = game.Workspace:Raycast(oldPos, rocket.position - oldPos, params)
        if results ~= nil then
            timePassed = 1000 --Boom
            rocket.n = results.Normal
        else
            local result = self:RayTestPlayers(oldPos, rocket.position - oldPos, server)
            if result ~= nil then
                timePassed = 1000
        ...
        if timePassed > 5 then
            ...
            self:DoExplosion(server, rocket.position, 15, 60)
        end
    end
end
