--[=[
    ...
    A collection of utility functions that can be used anywhere
]=]
...
--- Deserializes an Enum into from numeric value into its EnumItem.
...
--- Serializes an EnumItem or string Name into its numeric Value. Good for data compression
...
--- @return CFrame -- Random CFrame inside a BasePart
function Utils.getRandomInPart(part: BasePart): CFrame
...
--- Create a BodyGyro object that aligns a LocalPlayer's character to the camera
--- @return BodyGyro
...
--- Rounds a number to the number of decimal places defined
--- @return number -- Rounded number
function Utils.roundNumber(num: number, numPlaces: number): number
...
--- Get a CFrame rotation between 2 Vectors about an axis
--- @return CFrame -- rotated CFrame
function Utils.getRotationBetween(u, v, axis)
    local dot, uxv = u:Dot(v), u:Cross(v)
    if (dot < -0.99999) then return CFrame.fromAxisAngle(axis, math.pi) end
    return CFrame.new(0, 0, 0, uxv.x, uxv.y, uxv.z, 1 + dot)
end

--- Checks to see if a position is inside a box
--- @return boolean
function Utils.isInsideBox(position: Vector3, box: BasePart | {[string]: CFrame | Vector3}): boolean
	local v3 = box.CFrame:PointToObjectSpace(position)
	return (math.abs(v3.X) <= box.Size.X / 2)
		and (math.abs(v3.Y) <= box.Size.Y / 2)
		and (math.abs(v3.Z) <= box.Size.Z / 2)
end
